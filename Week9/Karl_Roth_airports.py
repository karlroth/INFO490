...
def get_flights(df):
    ...
    # your code goes here
    departures = df.groupby("Dest").size() #sum the dest col for each IATA value
    arrivals = df.groupby("Origin").size() #sume the origin col for each IATA value
    
    result["flights"] = departures.add(arrivals)
    result["flights"].fillna(1, inplace = True) #replaces the NaN ACY value with 1
...
def xml_city(url):
    ...
    # your code goes here
    soup = BeautifulSoup(url, "xml") #import as xml
    result = soup.AirportStatus.City.string #access <AirportStatus><City>"Name"</City></AirportStatus>
...
def xml_temp(url):
    ...
    # your code goes here
    soup = BeautifulSoup(url, "xml")#parse as xml
    
    result = soup.AirportStatus.Weather.Temp.string #access <AirportStatus><Weather><Temp>"Temperature"</Temp>
                                                    #                </Weather></AirportStatus>
                                                    
    result = re.search('[0-9]*[0-9]*[0-9]+', result).escape() #finds the first one-, two-, or three-digit number 
...
def add_city_temp(df):
    ...
    df["url"] = "http://services.faa.gov/airport/status/"+df[IATA] #creates url with appropriate IATA code
    df["city"] = df["url"].apply(xml_city()) #applies xml_city() to the entire "url" column
    df["temp"] = df["url"].apply(xml_temp()) #applies xml_temp() to the entire "url" column
    
    result = df["flights"]["city"]["temp"] #selects these three columns
    return result
...
# your code goes here
soup = BeautifulSoup(usairports, raw = True)

#finds all elements containing the word circle
circles = soup.findAll("circle")

#the colors we will be using
colors = ["#F1EEF6", "#D4B9DA", "#C994C7", "#DF65B0", "#DD1C77"] 

#loops through all of the cirlces
for c in circles:
         
        #make the circle proportional to the traffic
        c["r"] = top20.flights[c["id"]]/15000
        
        temp = top20.temp[c["id"]]
        ...
        #set the color based off of the temperature
        c["fill"] = color[color_class]
