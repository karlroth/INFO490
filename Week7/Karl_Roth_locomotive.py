...
def hypotheses(start=1, end=10001):
    ...
    #makes an array from start to end in integer increments.
...
def uniform_prior(hypotheses):
    ...
    '''
    #create an array of all ones divided by the length of hypotheses.
    length = len(hypotheses)
    result = np.ones_like(hypotheses)
...
def likelihood(data, hypotheses):
    ...
    array([ 0.,  0.,  0.,  0.,  0.])
    '''
    #returns likelihood as described above.
    result = (hypotheses > data).astype(np.float)
...
def posterior(data, hypotheses):
    ...
    #obtains and normalizes posterior by multiplying prior and likelihood. then dividing by the sum.
...
def estimate(posterior, hypotheses):
    ...
    2.25
    '''
    #returns the sum of the product of posterior and hypotheses
    result = posterior*hypotheses
    result = result.sum()
