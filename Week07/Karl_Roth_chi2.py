...
def get_expected(x):
    ...
    '''
    x = x.astype(np.float) #ensures x is a floating point array
    value = x.sum()/len(x)
    expected = np.empty(len(x)) #makes empty array of the same length
    expected[:] = value #sets all values of expected equal to value.
...
def get_diff(observed, expected):
    ...
    array([-1., -2., -3.])
    '''
    #subtract each element of expected from observed
...
def get_chi2(diff, expected):
    ...
    chi2 = diff*diff/expected #individual terms in the form of an array 
    
    chi2 = chi2.sum() #sum of all the chi2 terms
...
diff = get_diff(cancelled, expected) #For some reason the diff function is not working correctly
